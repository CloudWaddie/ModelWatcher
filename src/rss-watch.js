...
/**
 * Load configuration from rss-config.json
 * @returns {Object} Configuration object
 */
function loadConfig() {
  const configPath = './rss-config.json';
...
function convertToFixupx(text) {
  if (!text) return '';
  return text.replace(/xcancel\.com\//g, 'fixupx.com/').replace(/x\.com\//g, 'fixupx.com/').replace(/twitter\.com\//g, 'fixupx.com/');
}
...
function createNewPostsEmbed(username, posts) {
  ...
  return {
    ...
    embeds: [{
      title: '📡 New Posts from @' + username,
      description: `@${username} just posted ${posts.length} new tweet${posts.length > 1 ? 's' : ''} ${getRelativeTimestamp()}!`,
      color: 0x1DA1F2,
      fields,
      timestamp: new Date().toISOString(),
      footer: {
 ...
 */
async function main() {
  console.log('=== RSS Watcher (xcancel) ===');
  console.log('Starting feed check...');
  
  const config = loadConfig();
  const statePath = config.state?.file || './logs/rss-state.json';
